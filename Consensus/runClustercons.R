...
oo <- rbind(oo,data.frame(a=as.character(rep(alg[a],length(rownames(cr)))),b=as.numeric(rownames(cr)),c=as.numeric(table(rm[,1])),d=as.numeric(cr$rob),e=as.numeric(crScales)))
