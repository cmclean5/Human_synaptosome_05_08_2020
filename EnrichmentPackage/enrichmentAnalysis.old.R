...
#require("eva");      #GPD fit
#require("ismev");    #GPD fit
#require("fExtremes");#GPD fit
#require("MASS");     #Poission fit
...
#        out <- 1e+19
#    } else {
#        out <- -sum(dgpd( x=(data - u), scale=xi, shape=beta, log=TRUE) )
#    }
#    out
#}
    
#ML.H0.GPD <- function ( xVaR , data , u, k , xi ){   
#    (optimfit <- optim( par=c(xi,xVaR,u,k), fn=parloglik, data=data, method="BFGS" ))
#    xi.est   <- optimfit$par
#    llmax    <- -parloglik( par=c(xi.est), data=data )
#    list( xi=xi.est, VaR=xVaR, llmax=llmax, conv=optimfit$convergence )
#}

#rootfunc <- function (xVaR , exceedances, u, k, xi, global.max, P){
#    ML.H0.GPD( xVaR=xVaR, data=exceedances, u=u, k=k, xi=xi )$llmax - global.max + qchisq( (1-P) , df=1 )/2
#}

#q.GPD <- function(x ,VaR, exceedances, u, k, xi, global.max, P){
#    lower <- uniroot( f=rootfunc, interval=c(VaR/4 ,VaR), exceedances=exceedances, u=u, k=k, xi=xi, global.max=global.max, P=P ) 
#    VaR.lower <-  lower$root
#    upper <- uniroot( f=rootfunc, interval= c(VaR, 4*VaR), exceedances=exceedances, u=u, k=k, xi=xi, global.max=global.max, P=P ) 
#    VaR.upper <- upper$root
#    return ( c(VaR.lower, VaR, VaR.upper) )
#}

GPDFit <- function( data, P=0.95 ){

    #--- uses package ismav
    A=gpd.fit(data,quantile(data,P),show=F)
    #gpd.diag(A)

    sigma   = A$mle[1]
    epsilon = A$mle[2]
    mu      = as.vector(A$threshold)
    lambda  = A$rate
    Mean    = mu + sigma/(1-epsilon)
    return(Mean)
    #----
    
    #----- uses package eva
    #P     = 0.95  #confidence level
    #alpha = 0.975 #quantile

    #n     = length(data)       # size of dataset
    #u     = as.vector(quantile( data, P)) #Threshold

    #Nu    = sum(data > u)      # Number of exceendances

    #mod1  = gpdFit(data, threshold=u)  #GPD fit

    #global.max = as.vector(mod1$nllh.final)

    #xi    = as.vector(mod1$par.ests[1]) # xi   ==> sigma   ==> Scale
    #beta  = as.vector(mod1$par.ests[2]) # beta ==> epsilon ==> Shape
    #k     = ( 1 - alpha ) / (Nu/n)     
    #VaR   = u + beta * (k^(-xi)-1)/xi
    
    #exceedances = as.vector(mod1$exceedances)

    #gpdRl(z=mod1, period=VaR, conf=0.95, method="profile")
    
    #CI <- q.GPD( x=data, VaR=VaR, exceedances=exceedances, u=u, k=k, xi=xi, global.max=global.max, P=(1-P) ) 
    #return(CI)
    #----

}

PoissionFit <- function( data ){

    fit = fitdistr(floor(data),"Poisson")

    lambda = as.vector(fit$estimate)[1]

    n      = as.vector(fit$n)[1]

    #ciL    = lambda - 1.96 * sqrt( lambda/n )
    #ciU    = lambda + 1.96 * sqrt( lambda/n )
    
    ciL    = exp(log(lambda)-sqrt(1/(n*lambda)))
    ciU    = exp(log(lambda)+sqrt(1/(n*lambda)))

    return(c(lambda, ciL, ciU))
}


#entropy <- function( mu, F, Cn, N ){
...
for( a in 1:length(ALGS) ){
    ...
    str <- sprintf("%s/%s/permute_p_values_%s.csv",enrdir,ALGS[a],ANNO[Anno])
     ...
     if( file.exists(str) && file.info(str)$size!=0 ){
         ...
         colnames(DF) <- c("Fn","C","Cn","N","Mu","OR","CIl","CIu","Pv","Ap","PvALT","ApALT","Fe","E(Mu)","F","Alg","Fc")#"FeL","FeU")
...
if( run ){
    ...
    write.table(rank, "ranking.csv", sep="\t", row.names=F, col.names=T, quote=F)
    ...
    labs(x="log2(Fe)",y="Fraction of Enriched Communities",title="GO.BP")+
...
png(sprintf("%s/%sFe.png",plotDIR,subDIR[S]),width=WIDTH,height=HEIGHT,units="px");
print(gplot)
dev.off()
    ...
    gplot2 <- ggplot(df,aes(x=log(as.numeric(df$X)),y=as.numeric(as.vector(df$Y)),colour=df$ALG))+
        geom_line(size=as.numeric(as.vector(df$LSIZE)),alpha=as.numeric(as.vector(df$ALPHA)))+
        labs(x="log(log2(Fe))",y="Fraction of Enriched Communities",title="GO.BP")+
        theme(axis.title.x=element_text(face="bold",size=rel(1.5)),
              axis.title.y=element_text(face="bold",size=rel(1.5)),
              legend.text=element_text(face="bold",size=rel(LEGtextSize)),
              plot.title=element_text(face="bold",size=rel(1.5)),
...
png(sprintf("%s/%slogFe.png",plotDIR,subDIR[S]),width=WIDTH,height=HEIGHT,units="px");
print(gplot2)
dev.off()
...
png(sprintf("%s/%sFe_col.png",plotDIR,subDIR[S]),width=WIDTH,height=HEIGHT,units="px");
print(gplot3)
dev.off()
...
png(sprintf("%s/%slogFe_col.png",plotDIR,subDIR[S]),width=WIDTH,height=HEIGHT,units="px");
print(gplot4)
dev.off()
