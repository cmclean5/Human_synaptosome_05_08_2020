...
resdir1 <- sprintf("PLOTS/%s",subDIR[S])
...
targets = NULL
#targets = c("AD","HTN","PD","MS")

DF = massageData(dfPv,errorRate, targets, alpha=0.05)

DF[,3] = 2*DF[,3]

ReOrder = order(as.numeric(DF[,3]),decreasing=F)
DF = DF[ReOrder,]
DF$a = as.vector(factor(DF$a))
DF$b = as.vector(factor(DF$b))
DF$c = as.vector(factor(DF$c))
DF$d = as.vector(factor(DF$d))
DF$a = factor(DF$a, levels=unique(DF$a))

colours <- c("0"="#999999","1"="#436EEE")
target  <- unique(as.vector(DF[,1][grepl("HTN",as.vector(DF[,1]))]))
    ...
    labs(x="error Rate",y="Disease pairs",title="")+
        ...
        theme(axis.text.x = element_text(face="bold", size=rel(1.5)),
              axis.text.y = element_text(face=makeBold(DF[,1],target),size=rel(1.5)),
        ...
        #scale_y_discrete(limits = rev(levels(DF[,1])))


ggsave(gplot,file=sprintf("%s/randomEdges.svg",resdir1), width=WIDTHcm, height=2*HEIGHTcm, units="cm");

    png(sprintf("%s/randomEdges.png",resdir1), width=WIDTH, height=2*HEIGHT, units = "px")
print(gplot)
dev.off()
