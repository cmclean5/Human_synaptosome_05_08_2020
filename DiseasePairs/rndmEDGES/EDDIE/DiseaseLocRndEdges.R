...
generateRandomEdges <- function( GG, errorRate=seq(0,1,0.1), SEED=NULL ){
    ...
    for( i in 1:length(Mrand) ){
        ...
        #    rd <- sample_gnm(n=Ntot,m=Mtot,directed=F,loops=F)
            ...
            cat("Before:\n")
            cat("N: ", length(V(rd)), " E: ", length(E(rd)),"\n")
    ...
    for( e in 1:length(rd) ){
        ...
        for( k in 1:NELE ){
            ...
            if(DDints[k,1] != DDints[k,2]){
                overlap <- as.numeric(diseaseOverlap(rd[[e]],DDints[k,1],DDints[k,2],oo))
            }
...
## save output files
for(i in 1:length(DDINTS)){

    st1 = sprintf("%s_%g.csv.gz","sAB_random_separation",errorRate[i])
    fwrite(DDINTS[[i]],st1)   
